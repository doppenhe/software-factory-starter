...
7. Read `docs/agent-verification.md` before opening any pull request.
8. Read `docs/github-coordination.md` when the project uses GitHub issues, PRs, CI, or parallel agents.
9. Read `docs/contracts.md` when the task defines or changes a shape that more than one writer touches.
10. Read `docs/data-safety.md` when the project holds real data.
11. Read `docs/scheduled-work.md` when the task runs on a schedule rather than on demand.
12. Read `docs/ux-critic.md` when the task changes visible product UX.
13. If starter files do not exist, run `python3 scripts/init-project.py --name "<project name>"`.
14. Ask the user the questions in `prompts/project-intake.md`.
15. Create or update `state/PROJECT_STATE.md`.
16. Produce a SCOPE block and wait for confirmation.
17. After confirmation, create the PLAN.
18. Execute, assess, and resolve.
...
Skip any one and the loop becomes fragile. Knowledge without runtime is advice. Runtime without workflow is motion without judgment. Workflow without knowledge makes the agent re-derive the project every run. Shared work without coordination makes agents collide silently.

Knowledge inherits so that one fact has one home. Foundation files define identity, beliefs, and mission. Domain files define durable craft rules. Module files define product surfaces. Module files import domain files, domain files import foundation files, and no module repeats a foundation fact.

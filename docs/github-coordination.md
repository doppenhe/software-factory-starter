...
The canonical repository holds `docs/agent-operating-model.md` or this file, the decision log, and the shared contracts. Satellite repositories keep an `AGENTS.md` that links to the canonical model, plus their own decision log for decisions that only affect them. Epics live in the canonical repository; child task issues live in whichever repository owns the code.

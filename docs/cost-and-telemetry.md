...
Treat the roadmap as a portfolio. Each feature carries a build cost, an expected return, a payback horizon, and a risk. Sunset the losers. Weekly review beats quarterly planning once build cost and iteration cost collapse.

Build cost stops being an estimate once the spend log exists. Use the measured number, and use `templates/feature-economics.md` to record the bet.
